...
1.  You need the **nuScenes mini** dataset (a small, easy-to-use version).
2.  Create a folder named `data` right here in the main project folder (the same folder where `train.py` and `eval.py` live).
3.  Download the nuScenes mini dataset (you might need to create an account on their website) and extract the files into the `data` folder you just created. The folder structure should look something like `data/v1.0-mini/`.
...
---
**Congratulations!** You have now trained and tested an AI model that predicts human and vehicle movement!
